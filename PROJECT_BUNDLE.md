# DAINIK JAHAN - PROJECT BUILD ARCHIVE (ANTIGRAVITY DELIVERABLE)

This clean aggregate file hosts the complete, functional, multi-module production codebase for **Dainik Jahan**, a premium digital replica news portal matching **The Guardian**'s design language, equipped with Gemini AI editorial intelligence, responsive custom image generation, persistent drafts, WordPress Gutenberg block output creators, and a dedicated copy-paste WordPress theme.

---

## Workspace Directory Outline
```text
/
├── package.json
├── tsconfig.json
...
## 2. Server Configuration (`/tsconfig.json`)
```json
{
  "compilerOptions": {
    "target": "ES2022",
...
## 3. Bundler Configuration (`/vite.config.ts`)
```typescript
...
## 4. Metadata Settings (`/metadata.json`)
```json
{
  "name": "Dainik Jahan",
  "description": "A premium news portal replica of The Guardian with Gemini AI-powered news editing, custom image generation, and global RSS breaking news integration.",
...
## 7. Entrypoint Markup (`/index.html`)
```html
<!doctype html>
<html lang="en">
  <head>
    ...
    <title>Dainik Jahan - Guardian Replica News Portal</title>
  </head>
...
## 8. Express Backend Engine (`/server.ts`)
```typescript
...
## 9. React Startup Launcher (`/src/main.tsx`)
```typescript
...
## 10. Stylesheets and Fonts (`/src/index.css`)
```css
...
## 11. Core Portal & Editor Engine (`/src/App.tsx`)
```typescript
  ...
  return (
    <div className="flex flex-col gap-6 max-h-full">
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {/* Output Panel */}
        <div className="flex flex-col space-y-3">
          <label className="text-xs font-black uppercase text-gray-400 tracking-wider">Editorial Standard Output (Dainik Jahan)</label>
  ...
  const [drafts, setDrafts] = useState<NewsDraft[]>(() => {
    ...
    return saved ? JSON.parse(saved) : [
      ...
      },
      {
        ...
        content: "### মানসম্মত শিক্ষা নিশ্চিত করতে মেগা বাজেট\nআসন্ন অর্থ বছরে মাধ্যমিক ও উচ্চশিক্ষা খাতে অভূতপূর্ব মেগা স্কিম নেওয়ার ইঙ্গিত দিয়েছে পরিকল্পনা কমিশন। গ্রামীন স্কুল সমূহে ইন্টারнеটের ব্যবস্থা ও এআই ল্যাব স্থাপন করা হবে অগ্রাধিকার ভিত্তিতে।",
        category: "Economy",
...
## 12. WordPress Theme Style (`/wordpress-theme/style.css`)
```css
/*
Theme Name: Dainik Jahan - Guardian Replica
Theme URI: https://dainikjahan.com
Author: Dainik Jahan Editorial Platform Ltd
Author URI: https://dainikjahan.com
Description: A premium news portal replica of The Guardian with custom column layout, modern serif typography, and elegant color schemes matching the Dainik Jahan aesthetic.
Version: 1.0.0
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: news, blog, two-columns, custom-header, editorial, translation-ready

This theme is designed for clean copy-paste block publishing from the Dainik Jahan AI Portal.
*/
```

---

## 13. WordPress Theme Index (`/wordpress-theme/index.php`)
```php
<?php
/**
...
## 14. WordPress Theme Functions (`/wordpress-theme/functions.php`)
```php
<?php
/**
